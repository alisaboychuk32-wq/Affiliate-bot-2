...
class PaymentsManager {
  constructor(telegram, adminIds) {
    ...
    this.ymoneyWallet = process.env.YMONEY_WALLET || '';
    this.ymoneySecret = process.env.YMONEY_SECRET || '';
  }
  ...
  async init() {
    ...
    console.log('💳 Платежи: Stars ✅ | ЮMoney:', this.ymoneyWallet ? '✅' : '❌ (нужен YMONEY_WALLET)');
  }
  ...
  async recordPayment({ userId, username, firstName, productId, amount, currency, paymentId }) {
    const p = {
      id: Date.now(), userId, username, firstName,
      productId, amount, currency, paymentId,
  ...
  async confirmYmoney(label) {
    try {
      ...
      await this.recordPayment({
        userId: pending.userId, username: '', firstName: 'ЮMoney',
        productId: pending.productId, amount: pending.amount,
        currency: 'RUB', paymentId: label,
      });
  ...
  async getStats() {
    try {
      ...
      const byMethod = {
        ...
        ymoney: payments.filter(p => p.currency === 'RUB').length,
      };
      return { total: payments.length, revenue: Math.round(totalRevenue), activePremium, byMethod };
    } catch { return { total: 0, revenue: 0, activePremium: 0, byMethod: { stars: 0, ymoney: 0 } }; }
  }
}
