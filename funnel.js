...
class Funnel {
  ...
  async sendSubscribeGate(ctx) {
    ...
    await ctx.replyWithHTML(text, {
      ...Markup.inlineKeyboard([
        [Markup.button.url(`📢 Подписаться на канал`, `https://t.me/kanalKursov7`)],
  ...
  async sendWelcome(ctx, user) {
    ...
    const text =
      `🦊 *Привет, ${name}!*\n\n` +
      `Добро пожаловать к Алисе Бойчук!\n\n` +
    ...
    const keyboard = Markup.inlineKeyboard([
      ...
      [Markup.button.callback('🤖 Спросить ИИ', 'ask_ai'), Markup.button.callback('⭐ Premium', 'buy_premium')],
      [Markup.button.callback('📣 Разместить рекламу', 'buy_ads')],
      [Markup.button.callback('📣 Разместить рекламу', 'buy_ads')],
    ]);
