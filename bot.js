...
bot.action('admin_scheduler', async (ctx) => {
  ...
  await ctx.editMessageText(
    ...
    Markup.inlineKeyboard([
      ...
      [Markup.button.callback('Добавить курс сейчас', 'admin_add_course')],
      [Markup.button.callback('Назад', 'admin_back')],
    ])
  );
});
...
bot.action('admin_add_course', async (ctx) => {
  if (!ADMIN_IDS.includes(ctx.from.id)) return ctx.answerCbQuery('⛔');
  await ctx.answerCbQuery('Добавляю курс...');
  const result = await offersUpdater.addDailyCourse(true);
  await ctx.editMessageText(
    result.success
      ? 'Курс добавлен! ' + result.course.emoji + ' ' + result.course.name + '. Всего курсов: ' + result.total
      : 'Ошибка: ' + result.error,
    Markup.inlineKeyboard([[Markup.button.callback('Назад', 'admin_scheduler')]])
  );
});
...
async function main() {
  ...
  offersUpdater.startDailyUpdate();
