...
bot.action('buy_premium', async (ctx) => {
  ...
  const statusText = isPremium
    ? `✅ У тебя уже есть Premium!\nДействует до: _${new Date(info.expires_at).toLocaleDateString('ru-RU')}_\n\n`
    : '';

  await ctx.editMessageText(
    `⭐ *Premium доступ*\n\n${statusText}` +
    `Что входит в Premium:\n` +
    `🔥 Закрытые офферы (комиссия 70-100%)\n` +
    `🤖 ИИ-консультации без лимита\n` +
    `📊 Личная аналитика продаж\n` +
    `⚡ Первым узнаёшь о новых офферах\n\n` +
    `💰 *Выбери пакет:*`,
    {
      parse_mode: 'Markdown',
      ...Markup.inlineKeyboard([
        [Markup.button.callback('⭐ 1 месяц — 100 ⭐ Stars', 'pay_stars_premium_month')],
        [Markup.button.callback('💳 1 месяц — 299 ₽', 'pay_rub_premium_month')],
        [Markup.button.callback('⭐ 3 месяца — 250 ⭐ Stars', 'pay_stars_premium_3month')],
        [Markup.button.callback('💳 3 месяца — 699 ₽', 'pay_rub_premium_3month')],
        [Markup.button.callback('📚 Курс «Старт» — 500 ⭐', 'pay_stars_course_starter')],
        [Markup.button.callback('📚 Курс «Старт» — 1490 ₽', 'pay_rub_course_starter')],
...
async function main() {
  ...
  console.log('💳 ЮKassa:', process.env.YUKASSA_TOKEN ? 'активна' : 'нужен YUKASSA_TOKEN');
