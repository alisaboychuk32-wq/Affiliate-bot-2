...
const contentGen = new ContentGenerator(ADMIN_IDS, bot.telegram);
...
bot.action(/^step_(\d+)_(.+)$/, async (ctx) => {
  await ctx.answerCbQuery();
  await funnel.sendFunnelStep(ctx, parseInt(ctx.match[1]), ctx.match[2]);
});
...
bot.action('ask_ai', async (ctx) => {
  ...
  if (!isPremium && !hasLimit) {
    return ctx.editMessageText(
      '*ИИ-ассистент*\n\n' +
      'Ты использовал ' + AI_FREE_LIMIT + ' бесплатных вопросов сегодня.\n\n' +
      'Для безлимитного доступа оформи Premium:',
      { parse_mode: 'Markdown', ...Markup.inlineKeyboard([
        [Markup.button.callback('⭐ Оформить Premium', 'buy_premium')],
  ...
  const limitText = isPremium ? 'безлимит (Premium)' : ((AI_FREE_LIMIT - used) + ' из ' + AI_FREE_LIMIT + ' осталось');
  ...
  await ctx.editMessageText(
    ...
    '_Просто напиши вопрос_ 👇',
    { parse_mode: 'Markdown', ...Markup.inlineKeyboard([
      [Markup.button.callback('❌ Выйти из ИИ', 'ai_exit')],
    ])}
  );
});
...
bot.action('buy_premium', async (ctx) => {
  ...
  const statusText = isPremium
    ? 'Premium активен до: _' + new Date(info.expires_at).toLocaleDateString('ru-RU') + '_\n\n'
    : '';
...
bot.action(/^pay_stars_(.+)$/, async (ctx) => {
  await ctx.answerCbQuery();
  try {
    await payments.sendStarsInvoice(ctx, ctx.match[1]);
    await ctx.deleteMessage().catch(() => {});
  } catch (e) {
...
bot.action(/^pay_ymoney_(.+)$/, async (ctx) => {
  ...
  if (!process.env.YMONEY_WALLET) {
    return ctx.reply(
      '*Оплата через ЮMoney*\n\n' +
      (product ? product.name + ' — ' + product.price_rub + ' руб\n\n' : '') +
  ...
  await ctx.editMessageText(
    ...
    'Сумма: *' + link.amount + ' руб*\n\n' +
    '1. Нажми кнопку ниже\n' +
    '2. Оплати картой или с кошелька\n' +
    '3. Нажми Я оплатил',
    {
      parse_mode: 'Markdown',
      ...Markup.inlineKeyboard([
...
bot.action(/^ymoney_confirm_(.+)$/, async (ctx) => {
  ...
  for (const adminId of ADMIN_IDS) {
    try {
      await ctx.telegram.sendMessage(adminId,
        '*Заявка ЮMoney*\n\n' +
        'От: ' + ctx.from.first_name + ' (@' + (ctx.from.username || ctx.from.id) + ')\n' +
  ...
  await ctx.editMessageText(
    'Заявка отправлена на проверку!\n\nАдминистратор активирует доступ в течение нескольких часов.\n\nВопросы: @Imalisa_boychuk',
  ...
  if (result) {
    ...
    try {
      await ctx.telegram.sendMessage(result.userId,
        ...
        { ...Markup.inlineKeyboard([[{ text: 'Главное меню', callback_data: 'main_menu' }]]) }
      );
    } catch (e) {}
  } else {
...
bot.action('buy_ads', async (ctx) => {
  ...
  await ctx.editMessageText(
    ...
    { parse_mode: 'Markdown', ...Markup.inlineKeyboard([
      [Markup.button.callback('1 пост — 500 руб', 'ads_order_post_1x'), Markup.button.callback('3 поста — 1200 руб', 'ads_order_post_3x')],
      [Markup.button.callback('Неделя — 2500 руб', 'ads_order_post_week'), Markup.button.callback('Месяц — 8000 руб', 'ads_order_post_month')],
      [Markup.button.callback('🏠 Назад', 'main_menu')],
    ])}
  );
});
...
bot.action(/^ads_order_(.+)$/, async (ctx) => {
  ...
  await ctx.editMessageText(
    'Оформление заявки\n\nПакет: ' + pkg.name + ' — ' + pkg.price + ' руб\n\nНапиши текст рекламного поста (до 500 символов):',
...
bot.action('admin_ads', async (ctx) => {
  ...
  await ctx.editMessageText(
    'РЕКЛАМА И ПЛАТЕЖИ\n\n' +
    'Доход реклама: ' + stats.revenue + ' руб\n' +
    'Доход Premium: ' + payStats.revenue + ' руб\n' +
    'Активных Premium: ' + (payStats.activePremium || 0) + '\n\n' +
...
bot.action('admin_ads_pending', async (ctx) => {
  ...
  if (!pending.length) return ctx.editMessageText('Новых заявок нет!', Markup.inlineKeyboard([[Markup.button.callback('Назад', 'admin_ads')]]));
  for (const o of pending.slice(0, 5)) {
    await ctx.telegram.sendMessage(ctx.from.id,
      '#' + o.id + ' | ' + o.firstName + ' | ' + o.packageName + ' — ' + o.price + ' руб\n' + o.adText + '\nКонтакт: ' + o.contactInfo,
      { reply_markup: { inline_keyboard: [[{ text: '✅ Одобрить', callback_data: 'ad_approve_' + o.id }, { text: '❌ Отклонить', callback_data: 'ad_reject_' + o.id }]] } }
    );
  }
  await ctx.editMessageText('Заявки выше 👆', Markup.inlineKeyboard([[Markup.button.callback('Назад', 'admin_ads')]]));
});
...
bot.action('admin_ads_all', async (ctx) => {
  ...
  const rows = all.slice(-8).reverse().map(function(o) {
    return '• ' + o.firstName + ' — ' + o.packageName + ' (' + (o.status === 'pending' ? '⏳' : o.status === 'published' ? '📢' : o.status === 'approved' ? '✅' : '❌') + ')';
  }).join('\n') || 'Нет заявок';
  await ctx.editMessageText('Заявки:\n\n' + rows, Markup.inlineKeyboard([[Markup.button.callback('Назад', 'admin_ads')]]));
});
...
bot.action('admin_offers_update', async (ctx) => {
  ...
  await ctx.editMessageText(
    result.success ? 'Офферы обновлены! Всего: ' + result.total : 'Ошибка: ' + result.error,
...
bot.action('admin_scheduler', async (ctx) => {
  ...
  await ctx.editMessageText(
    'РАСПИСАНИЕ\n\nАвтопостинг: каждые 12ч\nСовет: 09:00 ежедневно\nРассылка: вс 10:00\nОфферы: пн 08:00\n\nОфферов в базе: ' + status.total,
    Markup.inlineKeyboard([
...
bot.action(/^broadcast_(.+)$/, async (ctx) => {
  ...
  await ctx.editMessageText('Введи текст рассылки:');
});
...
bot.on('text', async (ctx, next) => {
  ...
  // Реклама шаг 1
  if (ctx.session && ctx.session.adOrder && ctx.session.adOrder.step === 'text') {
  ...
  // Реклама шаг 2
  if (ctx.session && ctx.session.adOrder && ctx.session.adOrder.step === 'contact') {
    const order = await adsManager.createOrder({
      userId, username: ctx.from.username || '', firstName: ctx.from.first_name || '',
      packageId: ctx.session.adOrder.packageId, text: ctx.session.adOrder.adText, contactInfo: text,
    });
    ctx.session.adOrder = null;
    const pkg = adsManager.getPrices()[order.packageId];
    return ctx.reply(
      'Заявка отправлена!\n\n' + (pkg ? pkg.name + ' — ' + pkg.price + ' руб\n\n' : '') + 'Свяжемся по: ' + text,
      Markup.inlineKeyboard([[Markup.button.callback('🏠 Меню', 'main_menu')]])
    );
  }

  // Рассылка от админа
  if (ADMIN_IDS.includes(userId) && ctx.session && ctx.session.awaitingBroadcast) {
    ...
    if (target === 'channel_post') {
      try {
        ...
        await ctx.reply('✅ Пост отправлен!', Markup.inlineKeyboard([[Markup.button.callback('Назад', 'admin_back')]]));
      } catch (e) { await ctx.reply('Ошибка: ' + e.message); }
    } else {
      const result = await admin.sendBroadcast(ctx, text);
      await ctx.reply('Отправлено: ' + result.sent + ', ошибок: ' + result.failed,
  ...
  // ИИ режим
  if (ctx.session && ctx.session.aiMode) {
    const isPremium = await payments.isPremium(userId);
    if (!isPremium && !checkAiLimit(userId)) {
      ctx.session.aiMode = false;
      return ctx.reply('Лимит вопросов исчерпан. Оформи Premium!',
        Markup.inlineKeyboard([[Markup.button.callback('⭐ Premium', 'buy_premium')]]));
    }
...
bot.action('admin_content', async (ctx) => {
  if (!ADMIN_IDS.includes(ctx.from.id)) return ctx.answerCbQuery('⛔');
  await ctx.answerCbQuery();
  const enabled = contentGen.isEnabled();
  await ctx.editMessageText(
    'ГЕНЕРАТОР КОНТЕНТА ДЛЯ SEO\n\n' +
    'Статус: ' + (enabled ? 'Активен' : 'Нужен GROQ_API_KEY') + '\n\n' +
    'Яндекс Дзен: статья каждые 3 дня в 08:00\n' +
    'Яндекс Q: ответ каждые 2 дня в 09:00\n\n' +
    'Готовый текст приходит тебе в бот — копируй и публикуй!',
    Markup.inlineKeyboard([
      [Markup.button.callback('Статья для Дзен', 'gen_dzen'), Markup.button.callback('Ответ для Q', 'gen_q')],
      [Markup.button.callback('Назад', 'admin_back')],
    ])
  );
});

bot.action('gen_dzen', async (ctx) => {
  if (!ADMIN_IDS.includes(ctx.from.id)) return ctx.answerCbQuery('⛔');
  await ctx.answerCbQuery('Генерирую статью...');
  await ctx.editMessageText('Генерирую статью для Яндекс Дзен... Это займёт 10-20 секунд.');
  try {
    const topics = contentGen.getDzenTopics();
    const topic = topics[Math.floor(Math.random() * topics.length)];
    const article = await contentGen.generateDzenArticle(topic);
    await contentGen.sendContentToAdmin('dzen', topic, article);
    await ctx.editMessageText(
      'Статья готова! Проверь следующее сообщение.',
      Markup.inlineKeyboard([[Markup.button.callback('Назад', 'admin_content')]])
    );
  } catch (e) {
...
bot.action('gen_q', async (ctx) => {
  if (!ADMIN_IDS.includes(ctx.from.id)) return ctx.answerCbQuery('⛔');
  await ctx.answerCbQuery('Генерирую ответ...');
  await ctx.editMessageText('Генерирую ответ для Яндекс Q... Подожди 10-20 секунд.');
  try {
    const questions = contentGen.getQQuestions();
    const question = questions[Math.floor(Math.random() * questions.length)];
    const answer = await contentGen.generateYandexQAnswer(question);
    await contentGen.sendContentToAdmin('q', question, answer);
    await ctx.editMessageText(
      'Ответ готов! Проверь следующее сообщение.',
      Markup.inlineKeyboard([[Markup.button.callback('Назад', 'admin_content')]])
    );
  } catch (e) {
    await ctx.editMessageText('Ошибка: ' + e.message,
...
async function main() {
  ...
  contentGen.startAutoGeneration();
  ...
  console.log('ЮMoney: ' + (process.env.YMONEY_WALLET ? 'активна' : 'нужен YMONEY_WALLET'));
